...
| [110826-session-provenance](sources/110826-session-provenance.md) | source | Auto-distill scratch-log phiên b8afb386 ngày 11/08 — chuyển cả opencode lẫn openclaude sang DeepSeek-V4-Pro, tìm ra gốc là shim `openclaude-askctd.sh` hardcode `gpt-5.6-sol` với key rỗng nên THẮNG cả env lẫn profile `/provider` (đính chính ghi chép 09/08); và đo bác bỏ giả thuyết harness burn token: dự án có harness-stamp chỉ +4,5% token tươi mà cache hit lại cao hơn (47% vs 32%), thủ phạm thật là cache OpenClaude đóng băng ở 6.656 token trong khi context phình tới 29k |
| [120826-session-provenance](sources/120826-session-provenance.md) | source | Auto-distill scratch-log phiên b8afb386 ngày 12/08 — viết runbook dựng opencode chạy DeepSeek-V4-Pro cho máy khác (để ngoài repo, chmod 600, có key); và đo ra hợp đồng `orca orchestration send` đã đổi mà 3 skill orca trong repo chưa theo: `worker_done` nay đòi `--outcome`, cấm dùng chung `--payload` với cờ cấu trúc, và đòi cả `--dispatch-id` — skill dạy đúng lệnh hỏng nên mọi agent đọc nó đều sinh lại lỗi |
| [140826-session-provenance](sources/140826-session-provenance.md) | source | Auto-distill scratch-log phiên b8afb386 ngày 14/08 — regen trang sinh overstack sau bản vá egress-guard; nội dung mỏng vì agent chưa ghi why thủ công, why được trích tự động từ git |
| [150826-session-provenance](sources/150826-session-provenance.md) | source | Auto-distill scratch-log phiên b8afb386 ngày 15/08 — truy lỗi `[Content blocked by provider safety filter]` ở dự án vms: chuỗi đó do openclaude tự chèn khi provider trả HTTP 200 kèm `finish_reason: content_filter`, tức lọc ĐẦU RA chứ không chặn đầu vào; tách từng thành phần context bắn lại endpoint đều không tái hiện được nên nguyên nhân vẫn mở, nhưng lộ ra hook anti-idle chỉ nhận diện đúng một chuỗi refusal cứng nên bỏ sót ca cùng chữ ký usage=0 |
| [170826-session-provenance](sources/170826-session-provenance.md) | source | Auto-distill scratch-log phiên b8afb386 ngày 17/08 — xoay key ctd-opus lần hai sau khi key cũ bị thu hồi, đồng bộ key + baseURL `openai.azure.com/openai/v1` qua 5 nơi (secrets, opencode.json, profile openclaude, 2 runbook máy khác) và kiểm chứng cả hai công cụ chạy lại được trên DeepSeek-V4-Pro |
| [180826-session-provenance](sources/180826-session-provenance.md) | source | Auto-distill scratch-log phiên b8afb386 ngày 18/08 — bác giả thuyết `[Content blocked by provider safety filter]` là rate limit (429 đi đường RateLimitError riêng, có retry; chuỗi đó chỉ sinh từ `finish_reason` content_filter/safety, và một lần chặn có `output_tokens=10` chứng tỏ model đã sinh chữ rồi bị cắt); đồng thời tìm ra openclaude nén context ở 79k–187k trong khi giới hạn thật của DeepSeek-V4-Pro là 1.048.576 token, gốc là Azure `/models` không khai context window nên openclaude đoán bừa — vá bằng `CLAUDE_CODE_AUTO_COMPACT_WINDOW` trong shim |
